...
class FileStateData(NamedTuple):
    current_line: str
    line_number: int
    spos: int
    epos: int
...
class FileHandler:
    ...
    def freeze_position(self, spos: int) -> FileStateData:
        """
        ...
        return FileStateData(self.current_line, self.line_number, spos, self.pos)
...
class ParsingState:
    ...
    def reset(self) -> None:
        ParsingState.__init__(
            self,
        )  # we only reset PARSING state, not subclasses stuff!!
